  ...
  <body data-page="mypage-only">
    ...
    <div class="modal" id="authModal" aria-hidden="true">
      <div class="modal__content auth-modal">
        ...
        <p class="subtitle auth-desc">이메일과 비밀번호로 로그인하거나 소셜 계정으로 간편하게 로그인하세요.</p>
        <div class="auth-fields">
          <label class="caption" for="authEmail">이메일</label>
          <input class="auth-input" type="email" id="authEmail" placeholder="you@example.com" />
          <label class="caption" for="authPassword">비밀번호</label>
          <input class="auth-input" type="password" id="authPassword" placeholder="비밀번호" />
          <!-- 체크박스 위치 조정: 픽셀 값은 style.css의 .remember-checkbox { transform: translateX(...) }에서 변경하세요 -->
          <div class="auth-checks">
            <label style="display:flex; flex-direction:row; align-items:center; gap:4px; font-size:14px; margin:0;">
              <input class="remember-checkbox" type="checkbox" id="rememberMe" /> 로그인 정보 기억
            </label>
          </div>
          <div class="auth-links">
            <a href="#" class="caption" style="color:var(--muted); text-decoration:underline;">비밀번호 찾기</a>
          </div>
          <button class="btn primary" id="authSubmit" type="button">로그인</button>
          <div style="text-align:center; margin:12px 0; font-size:13px; color:var(--muted);">또는</div>
          <div class="social-row">
            <button class="btn ghost" id="socialKakao">Kakao로 계속하기</button>
            <button class="btn ghost" id="socialFacebook">Facebook로 계속하기</button>
          </div>
          <p class="caption" style="margin-top:10px;">Supabase 인증으로 로그인/회원가입됩니다.</p>
        </div>
      </div>
    </div>
